...
# =============================================================================
# Author: Haodong Tian
# Description: REGENIE step 1 — fits a whole-genome regression model on the
#              full QC-passed genotype data for variance component estimation,
#              applied to all 200 liver MRI radiomics features.
...
#$ -l h_vmem=8G
#$ -o logs/Step1_new.$JOB_ID.out
#$ -e logs/Step1_new.$JOB_ID.err

# Phenotype file versions:
#my_liver_UKB_more_covar.txt     # water-phase radiomics features only (original)
#my_liver_UKB_more_covar_new.txt # combined water + in-phase features; QC-filtered to EUR individuals with improved covariates
...
mkdir -p ${wdir}/Step1_new

bed=/path/to/server/resources/regenie/ukb_cal          # v2 bed+bim+fam
snplist=/path/to/server/resources/regenie/qc_pass.snplist  # post-QC variants (509,528 variants)
samplist=/path/to/server/resources/regenie/qc_pass.id      # post-QC individuals (488,371 subjects)
